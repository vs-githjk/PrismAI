...
// Workspace default persona is preset-only (no custom) so an admin can't
// inject arbitrary prompt text into every member's meetings.
              ...
              <div className="flex items-center justify-between border-t border-[color:var(--db-border)] pt-3">
                {wsDetails.your_role === 'owner' ? (
                  <button
                    ...
                    className="rounded-lg border border-red-400/30 bg-red-400/[0.07] px-3.5 py-1.5 text-[11px] font-semibold text-red-300/80 transition hover:border-red-400/55 hover:bg-red-400/[0.12] hover:text-red-200"
                  >
                    Delete workspace
                  </button>
                ) : (
