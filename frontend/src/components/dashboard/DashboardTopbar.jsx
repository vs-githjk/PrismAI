...
export default function DashboardTopbar({ title, searchValue, onSearchChange, actions = null, status = null, signedOut = false, onLockedFeature, onBack = null }) {
  ...
  return (
    <header className="dashboard-topbar dashboard-island z-30 flex items-center gap-4 px-6">
      ...
      <div className="flex h-11 w-[clamp(200px,30vw,380px)] shrink-0 items-center gap-2.5 rounded-full border border-[color:var(--db-border)] bg-[var(--db-fill)] px-4 transition focus-within:border-cyan-400/45 focus-within:bg-[var(--db-fill-strong)]">
        <input
