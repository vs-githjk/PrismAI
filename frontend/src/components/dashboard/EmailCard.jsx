...
export default function EmailCard({ email, gmailConnected = false, suggestedEmails = [], onSave, viewerName = '', meetingId, transcript = '', result = null }) {
  ...
  const actionBtn =
    'flex items-center gap-1.5 rounded-lg border border-[color:var(--db-border)] bg-[var(--db-fill)] px-2.5 py-1.5 text-[11px] font-medium text-[color:var(--db-text-muted)] transition hover:border-[color:var(--db-border-strong)] hover:text-[color:var(--db-text)]'

  return (
    <section className={`${glassCard} p-5`} style={cardGlowStyle}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold tracking-[-0.01em] text-[color:var(--db-text)]">Follow-up email</h2>
          <p className="mt-1 text-xs font-medium text-[color:var(--db-text-faint)]">
            {editing
              ? 'Editing draft'
      ...
      {showSendForm && !editing && (
        <div className="mb-3 overflow-hidden rounded-lg border border-[color:var(--db-border)]">
          <div className="space-y-2 px-3 py-2.5">
            <label className={subtleText}>To: <span className="text-[color:var(--db-text-faint)]">(separate with commas)</span></label>
            <input
              type="text"
              value={toInput}
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className={subtleText + ' transition hover:text-[color:var(--db-text-soft)]'}
              >
                Cancel
              </button>
              <button
      ...
      {editing ? (
        <div className="space-y-2.5">
          <input
            ...
            placeholder="Subject"
            className="w-full rounded-lg border border-[color:var(--db-border)] bg-[var(--db-fill)] px-3 py-2 text-[15px] font-semibold text-[color:var(--db-text)] outline-none placeholder:text-[color:var(--db-text-faint)] focus:border-cyan-400/40"
          />
          <textarea
            value={draftBody}
            ...
            className="w-full resize-y rounded-lg border border-[color:var(--db-border)] bg-[var(--db-fill)] px-3 py-2 text-sm leading-7 text-[color:var(--db-text-soft)] outline-none placeholder:text-[color:var(--db-text-faint)] focus:border-cyan-400/40"
          />
          <div className="flex justify-end gap-2">
            <button
              type="button"
        ...
        <>
          {view.subject && (
            <p className="text-[15px] font-semibold leading-snug text-[color:var(--db-text)]">{view.subject}</p>
          )}
          <p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-[color:var(--db-text-soft)]">{view.body}</p>
        </>
      )}
    </section>
  )
