...
export default function AgentTags({ agents, totalAgents }) {
  ...
  return (
    <div className="rounded-[24px] border border-[color:var(--db-border)] p-4 animate-fade-in-up card-delay-0"
      ...
      <div className="flex flex-col gap-3">
        ...
        {/* Orchestrator badge */}
          <div className="flex items-center gap-2 pr-4 sm:border-r border-[color:var(--db-border)]">
            <div className="w-8 h-8 rounded-xl bg-[var(--db-fill-strong)] border border-[color:var(--db-border-strong)] flex items-center justify-center flex-shrink-0">
            <svg className="w-3.5 h-3.5 text-[color:var(--db-text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex flex-wrap gap-2 text-[10px] text-gray-500">
          <span className="px-2.5 py-1 rounded-full border border-[color:var(--db-border)] bg-[var(--db-fill)]">Live stream order depends on agent completion</span>
          <span className="px-2.5 py-1 rounded-full border border-[color:var(--db-border)] bg-[var(--db-fill)]">Outputs are generated from transcript evidence</span>
        </div>
      </div>
    </div>
  )
