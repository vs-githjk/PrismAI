...
- **Live-meeting fix (2026-07-25) — Cartesia concurrency:** two real meetings ran; the architecture held end to end (Flux ears, Silero VAD, barge-in twice, two-channel brain, tools, Cartesia mouth). Top blocker was `429: concurrency limit of 2` killing multi-sentence replies. Root cause was our frame shape, not the plan tier: one `TTSSpeakFrame` per chunk == one Cartesia context per chunk. A reply is now bracketed as one pipecat "response" so all its chunks share ONE context. Details in [phase 5](voice-agent-build-plan-phase5.md) §8.
- **Flags flipped to default-ON (2026-07-26):** `PRISM_TWO_CHANNEL` and `PRISM_ENGAGEMENT_GATE` now default ON, per each phase's own plan ("flip it for the first live join, then it becomes the default"). Owner's call, so the test matrix exercises the new brain + gate instead of the legacy paths it was silently testing. `=0` in the Render dashboard forces either back without a deploy; the legacy code is untouched and remains the rollback until the §7 demolition ruling. Note the flag had a SECOND read site (`voice_channel.on_eager_turn`) with its own default — both now route through `realtime_routes._two_channel_on()`.
- **Deploy host is RENDER, not Railway.** `render.yaml` is live and load-bearing (it pins `PYTHON_VERSION 3.11.0`, which is what gets Silero onto prod, and sets `WEBHOOK_BASE_URL=https://meeting-copilot-api.onrender.com`). A 2026-07-25 session handoff claimed Railway and called `render.yaml` vestigial — that was wrong; there is not a single Railway reference anywhere in the repo. Env vars are set in the Render dashboard.
...
- **Phase 5 dry build COMPLETE (2026-07-22) — feel tuning, §7 demolition HELD:** the knob layer + real-audio barge-in are in. New `backend/voice/tuning.py` = the §5 knob table, every dial env-overridable and seeded from Curio's `tuning.py` where an equivalent exists (`FLUX_*`, `TTS_*`, and Curio's `BACKCHANNEL_MIN_WORDS=3` ported as **`LATE_INTERRUPT_MIN_WORDS`** — its *value*, never its mechanism; Curio's `USER_TURN_SETTLE_SECS` has no home here because our pipeline has no user-turn aggregator, Q1). New `backend/voice/barge.py` = Silero's one signal turned into two features: **§1 barge-in** (`VADProcessor(SileroVADAnalyzer)` + `BargeInGate` between transport and Flux; a burst must still be running `BARGE_MIN_SPEECH_MS` after it started before it fires, so "yeah"/"mm-hm" don't kill the reply; firing = `broadcast_interruption()` + a new `{"type":"stop"}` the speaker page uses to drop already-scheduled Web Audio buffers) and **§2 the politeness gap** on `RoomAudio.quiet_seconds()` instead of transcript timestamps (50ms poll; `[voice-gap] SUMMARY` logs median/p90 + a `source=vad|transcript` split every 10 waits — the before/after report §2 promised). **Flux's `should_interrupt` is now OFF whenever Silero loaded** (fork ④: Silero owns barge-in; Flux interrupts on *any* turn start, the exact behaviour §1 fixes) and ON as the fallback. **Late interrupt:** a sub-threshold burst whose transcript is ≥3 words interrupts after the fact. **Mute + spoken "stop"** call `hard_stop()` pre-gate — which surfaced a real bug: the stop detector read `segment["words"]`, always empty on the Flux path, so "stop" was undetectable once the ears were swapped (now falls back to `segment["text"]`). **§3 speculation:** `EagerEndOfTurn` → a voice-channel call started early with its deltas buffered, adopted by the real post-EndOfTurn path only on an exact text match, thrown away on `TurnResumed`/mismatch/TTL; gated by the new read-only `gate.might_engage` so it can never consume the bare-name arm window, and **dormant until `PRISM_FLUX_EAGER_EOT_THRESHOLD` is set** (Flux's default is off). **Stopwatch is finally end-to-end** — a per-bot open-turn slot joins ears (t0/t1, one instant on Flux), voice channel (t2), mouth (t3/t4), so §3's median review is possible; an interrupted utterance drops its turn so the next can't inherit a poisoned t3. **§4:** Cartesia speed/volume/emotion knobs wired; `_spoken_condense` limits are knobs at the old 3/340 (the streaming mouth removes the pacing constraint that set them, but re-picking needs real audio). **Degrades on purpose:** no onnxruntime → no Silero → Flux interruption + transcript-timestamp gap, backend still boots (prod pins Python 3.11). **§7 demolition deliberately HELD pending owner ruling:** the transcript-timestamp gap is not legacy (it's the fallback for every non-voice bot), and phases 2–4's deferred demolitions are the only rollback until one real meeting happens. **NOT run in a real meeting.** Plan: [phase 5](voice-agent-build-plan-phase5.md).
- **Phase 4 dry build COMPLETE — additive, behind `PRISM_ENGAGEMENT_GATE` (2026-07-18):** the single engagement gate is built alongside the legacy wake/solo/ambient machinery, not in place of it (same demolition-wait principle). New `backend/voice/gate.py` = the ONE "speak now?" decision: `decide(bot_id, state, text, speaker)→(speak, command)` per finished turn + `propose(bot_id, state, candidate)` for the watchers. Two behaviors, one toggle: **Manual** (wake-word only, even solo) / **Auto** (default; addressed→answer, solo→free conversation, group→no per-utterance replies). Mode never auto-switches (`engagement_mode`, migrated from legacy `manual_mode`/`mode`). **Consolidation flagged:** the per-utterance "ambient judge replies to a statement" collapses into the proposer→gate path (the idea engine is already the contribution generator) — so Auto-group interjection comes ONLY through `propose()` (drift checker + idea engine rewired to `_proactive_send`→`gate.propose`), no second murkier judge, no consent question (fork ①). `propose()` is chat-primary (today's proactive nudges are meta-hints that read wrong spoken; `speak=True` opts in — spoken-proactive is a future-ideas tuning knob). Wiring: `_handle_realtime_payload` calls `gate.decide()` behind the flag (bypassing the legacy fragment-gluing detection, which Flux makes dead); `/bot/{id}/mode` + `/join-meeting` + the seed accept `auto`/`manual` (legacy `utterance`/`autonomous` still mapped); frontend NewMeetingPanel toggle relabeled Auto/Manual (default Auto). **Verified:** backend imports clean; gate self-check passes (manual wake-only, auto-solo free-flow, auto-group addressed-only, bare-name arm, mute hard-stop). **NOT run in a real meeting** (deferred to the one end-of-phases test). **Deferred to demolition:** delete `_solo_*`/`PRISM_SOLO_FREEFLOW`, the ambient consent funnel (`interject`/`offer_decider`/`classify_consent`/`_ambient_speak_offer`/`_ambient_run_delivery`), fragment-gluing wake code, direct-speak in the watchers. Plan: [phase 4](voice-agent-build-plan-phase4.md).
