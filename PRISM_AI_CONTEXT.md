...
Closes open item D1 of the May 28 personas spec. The live meeting bot now reads the owner's persona (personal override → workspace default → ''), where before `realtime_routes.py` had zero persona references. Architecture: persona text lands in the cached static system prefix (`_build_static_prefix`) so it rides Groq prompt caching at ~0 marginal cost per command. A tool-aware wrapper (`persona_suffix_agentic`) fences tone from tool-calling decisions so a "concise" preset can't suppress a real Gmail/Calendar tool call. Resolution reuses the `user_settings` row already in hand — no second DB fetch. Survives the Haiku fallback for free (fallback concatenates all system messages, persona in `messages[0]` carries). `workspace_id` plumbed through `JoinMeetingRequest` → `bot_store` → resolver so workspace-default personas reach the bot. 14 new tests across persona/prompt-structure/realtime suites. Merged to main as commit `b1ac47a`.

### Added Jun 3 2026 — Persona identity (names + greetings + wake aliases) + persona icons in picker + notifications plan

**Backend (`7a57831` to main):** every non-default preset now renames the in-meeting bot to a Prism-family name — `concise → Flash`, `formal → Crystal`, `cheeky → Glint`, `socratic → Echo`, `warm → Glow`, `analytical → Spectrum`. Defined in `personas.py PERSONA_NAMES` / `PERSONA_GREETINGS`. The bot's intro chat message (`recall_routes._send_bot_intro`) picks a hardcoded greeting per preset; the live-link + consent lines are appended verbatim regardless. The persona name is also injected into the cached static system prefix (`_build_static_prefix` — `"Your name in this meeting is X..."`) so the bot self-identifies as that name in every LLM reply. **Wake words are per-bot:** the bot responds to its persona name (e.g. "Flash, summarize…") in addition to the always-on `Prism / PrismAI` aliases — `_BOT_WAKE_ALIAS[bot_id]` populated by `_get_settings_for_bot`, compiled patterns cached in `_WAKE_PATTERN_CACHE`, cleared in `cleanup_bot_state`. Custom personas keep the "Prism" name + default greeting (custom is tone-only). The Recall participant display name in the meeting sidebar stays `PrismAI`. Resolution helper `personas.persona_identity_resolved` returns `(name, text, preset)` in one call, reuses the user_settings row already in hand. Dropped the `✓ ` prefix on the bot's chat replies. Proactive nudges + `knowledge_proactive` surfacing use the active persona name when telling the user how to invoke the bot ("Say 'Flash, summarize…'"). 17 new unit tests.
